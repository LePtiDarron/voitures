...
const CarsList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BACK_URL}/api/cars`, {
          params: { page }
        });
  ...
  return (
    <div className='page-and-footer'>
      <div className="main-page">
        {cars.map((car) => (
          <Link to={`/mod/cars/${car.id}`} key={car.id} className="car-item">
      ...
      <div className='admin-button-container'>
        <Link to="/mod/cars/create">
          <button className='admin-button'>Ajouter une voiture</button>
        </Link>
      </div>
      <Footer />
    </div>
